...
`ActorInteractionRenderer` is a reusable, explicit-input boundary for actor references. Locked module windows retain `NoInputs`; after content rendering, the helper creates a transparent window exactly over each registered clickable row. `ActorTargetingService` re-resolves the game-object ID at click time, checks current validity/targetability and sets `ITargetManager.Target`. No world-click simulation or native target pointer is retained.
...
Sentinel Core's `ConfigurationCoordinator` loads and normalizes schema version 5, saves ordinary settings immediately, debounces drag saves, flushes due saves during draw and performs a final flush during disposal. Module anchors, scale, width, bar height, MP modes, player-cast fields, Focus Target's Target fields, HP colour modes, background/border appearance, awareness settings, native-target offsets and camera settings all persist. Schema-1/2 migration preserves existing positions, scale, colours, enabled states, awareness and camera settings while deriving marker/shield presentation from legacy switches; schema-3 migration maps the former Player `ShowMp` switch to the new MP presentation mode; schema-5 adds new fields through normalized defaults without resetting existing data.
